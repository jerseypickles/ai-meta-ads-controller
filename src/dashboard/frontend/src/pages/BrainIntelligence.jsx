...
function AresPanel({ data, loading, running, onRun, onRefresh }) {
  ...
  const CboCard = ({ title, color, stats, adsets, sectionKey }) => (
    <div style={{ position: 'relative', marginBottom: 16 }}>
      <div style={{ position: 'absolute', left: -27, top: 12, width: 12, height: 12, borderRadius: '50%', background: color }} />
      <div style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-md)', padding: '14px 18px', border: `1px solid ${color}20` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <div style={{ fontSize: '0.72rem', color, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{title}</div>
          <div style={{ fontSize: '1.5rem', fontWeight: 800, color: roasColor(stats.roas || 0) }}>{stats.roas || 0}x</div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 10 }}>
          {[
            { v: `$${stats.revenue_7d || 0}`, l: 'Revenue' },
            { v: `$${stats.spend_7d || 0}`, l: 'Spend' },
            { v: stats.purchases_7d || 0, l: 'Compras' },
            { v: stats.cpa ? `$${stats.cpa}` : '—', l: 'CPA' }
          ].map((s, i) => (
        ...
        {(adsets || []).length > 0 && (
          <>
            <div onClick={() => toggle(sectionKey)} style={{ cursor: 'pointer', userSelect: 'none', fontSize: '0.72rem', color: 'var(--text-muted)', paddingTop: 8, borderTop: '1px solid var(--border-color)' }}>
              {expandedSections[sectionKey] ? '▾' : '▸'} {(adsets || []).length} clones ({(adsets || []).filter(a => a.spend_7d > 0).length} con spend)
            </div>
            {expandedSections[sectionKey] && (
              <div style={{ marginTop: 8 }}>
                {(adsets || []).sort((a, b) => b.roas_7d - a.roas_7d).map((a, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '4px 0', borderBottom: '1px solid var(--border-color)', fontSize: '0.7rem' }}>
                    <span style={{ color: 'var(--text-primary)' }}>{a.adset_name}</span>
                    <span style={{ display: 'flex', gap: 12, color: 'var(--text-muted)' }}>
                      <span>${a.spend_7d}</span>
                      <span>{a.purchases_7d}c</span>
                      <span style={{ color: roasColor(a.roas_7d), fontWeight: 600 }}>{a.roas_7d}x</span>
                    </span>
                  </div>
                ))}
              </div>
    ...
    <div className="agent-panel">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: '1.15rem', fontWeight: 600 }}>Ares — Duplicador CBO</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>2 campanas CBO — ganadores probados + nuevos ganadores</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={i} style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: '10px', textAlign: 'center' }}>
            <div style={{ fontSize: '1.1rem', fontWeight: 700, color: s.c || 'var(--text-primary)' }}>{s.v}</div>
            <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)' }}>{s.l}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ borderLeft: '2px solid #ef4444', paddingLeft: 20, marginLeft: 8 }}>
          ...
          <div style={{ position: 'relative', marginBottom: 16 }}>
            ...
            <div onClick={() => toggle('candidates')} style={{ cursor: 'pointer', userSelect: 'none', display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 500 }}>
                {expandedSections.candidates ? '▾' : '▸'} Candidatos ({candidates.length})
              </span>
            </div>
                  ...
                  <div key={i} style={{ position: 'relative', marginBottom: 4 }}>
                    ...
                    <div style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: '8px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '0.63rem', color: 'var(--text-muted)' }}>${c.spend_7d} | {c.purchases_7d} compras | freq {c.frequency}</div>
                      </div>
        ...
        {dups.length > 0 && (
          <div style={{ position: 'relative' }}>
            ...
            <div onClick={() => toggle('history')} style={{ cursor: 'pointer', userSelect: 'none', fontSize: '0.8rem', color: 'var(--text-secondary)', fontWeight: 500, marginBottom: 8 }}>
              {expandedSections.history ? '▾' : '▸'} Historial ({dups.length})
            </div>
            {expandedSections.history && (
              <div style={{ borderLeft: '1px solid var(--border-color)', paddingLeft: 16, marginLeft: 4 }}>
                {dups.map((d, i) => (
                  <div key={i} style={{ background: 'var(--bg-secondary)', borderRadius: 'var(--radius-sm)', padding: '6px 12px', fontSize: '0.68rem', marginBottom: 4, display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-primary)' }}>{d.original_name} → {d.clone_name}</span>
                    <span style={{ color: 'var(--text-muted)' }}>{(d.roas_at_dup || 0).toFixed(1)}x | {new Date(d.executed_at).toLocaleDateString('es', { month: 'short', day: 'numeric' })}</span>
                  </div>
                ))}
              </div>
            )}
