 ...
 * Usuario aprueba la propuesta. Ejecuta creación completa:
...
router.post('/approve', async (req, res) => {
  try {
    ...
    // campaign_id/name come from the parent result, attached by frontend
    ...
    if (!pixelInfo) {
      return res.status(400).json({ error: 'No se encontró pixel_id en la cuenta. Se necesita al menos un ad set existente.' });
    }
    if (!pageId) {
      return res.status(400).json({ error: 'No se encontró page_id de Facebook.' });
    }
    if (!websiteUrl) {
      return res.status(400).json({ error: 'No se encontró link_url en ningún ad existente. Se necesita al menos un ad con URL de destino.' });
    }
    steps[steps.length - 1].status = 'done';

    // Step 3: Upload images to Meta (parallel uploads — feed + stories pairs)
    ...
    const uploadResults = await Promise.allSettled(
      proposal.selected_creatives.map(async (sel) => {
        ...
        if (!asset) {
          throw new Error(`Asset ${sel.asset_id} no encontrado`);
        }
    ...
    if (uploadedAssets.length < 2) {
      return res.status(400).json({ error: `Solo se pudieron subir ${uploadedAssets.length} imágenes. Se necesitan mínimo 2.`, errors });
    }
    ...
    try {
      ...
      return res.status(500).json({ error: `Error creando ad set en Meta: ${metaDetail}`, errors, steps });
    }
    steps[steps.length - 1].status = 'done';
    steps[steps.length - 1].adset_id = adSetResult.adset_id;

    // Step 5: Create ad creatives + ads (feed 1:1 + stories 9:16 per variant)
    const totalVariants = uploadedAssets.reduce((sum, { creative_config, pairedAsset }) => {
      ...
      const multiplier = pairedAsset ? 2 : 1; // feed + stories if paired
    ...
    for (const { asset, pairedAsset, creative_config } of uploadedAssets) {
      // Support multiple headlines/bodies arrays (new) or single headline/body (legacy)
      ...
      // Create one ad per headline+body pair (zip by index)
    ...
    if (createdAds.length === 0) {
      return res.status(500).json({ error: 'No se pudo crear ningún ad', errors, steps });
    }

    // Step 6: Activate ad set + all ads (parallel activation)
    steps.push({ step: 'activate', status: 'running' });
    try {
      // Activate all ads in parallel first
    ...
    // Step 7: Register AICreation for lifecycle tracking
    ...
    try {
      aiCreation = await AICreation.create({
        ...
        learning_ends_at: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000), // 3 days
        lifecycle_actions: [{
